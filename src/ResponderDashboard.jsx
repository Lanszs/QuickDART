...
const ResponderDashboard = ({ onLogout }) => {
    ...
    const fetchData = async () => {
        try {
            ...
            const repResponse = await fetch(`${API_URL}/api/v1/reports?team_id=${currentTeamId}`);
            if (repResponse.ok) setReports(await repResponse.json());

            // 2. Fetch Team Info
            const resResponse = await fetch(`${API_URL}/api/v1/resources`);
            if (resResponse.ok) {
            ...
            const depResponse = await fetch(`${API_URL}/api/v1/teams/${currentTeamId}/deployments?status=Active`);
    ...
    useEffect(() => {
        ...
        const fetchInitialData = async () => {
            try {
                ...
                // 1. Reports
                const repRes = await fetch(`${API_URL}/api/v1/reports?team_id=${currentTeamId}`);
                if (repRes.ok) {
                ...
                // 2. Team Info
                const resRes = await fetch(`${API_URL}/api/v1/resources`);
                if (resRes.ok) {
                ...
                const depRes = await fetch(`${API_URL}/api/v1/teams/${currentTeamId}/deployments?status=Active`);
                if (depRes.ok) {
                    setActiveDeployments(await depRes.json());
                }

                // 4. Chat History
                const chatRes = await fetch(`${API_URL}/api/v1/chat/history/team_${currentTeamId}`);
                if (chatRes.ok) {
        ...
        socket.on('resource_updated', (update) => {
            if (update.type === 'team' || update.type === 'asset') {
                ...
                fetch(`${API_URL}/api/v1/resources`)
                    .then(res => res.json())
                    .then(data => {
                        const team = data.teams.find(t => t.id === parseInt(currentTeamId)); 
                        setMyTeam(team);
        ...
        socket.on('deployment_completed', () => {
            fetch(`${API_URL}/api/v1/teams/${currentTeamId}/deployments?status=Active`)
                .then(res => res.json())
   ...
   const isWithinRadius = (report, teamOverride = null) => {
    ...
    const handleRespondNow = async () => {
        ...
        try {
            const res = await fetch(`${API_URL}/api/v1/teams/${myTeam.id}/deployments`, {
                method: 'POST',
    ...
    const confirmCompleteMission = async () => {
        ...
        try {
            const res = await fetch(`${API_URL}/api/v1/deployments/${deployment.id}/complete`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' }
            });
    ...
    const handleBulkDeleteAssets = async () => {
        ...
        try {
            ...
            const deletePromises = inventorySelection.map(id => 
                fetch(`${API_URL}/api/v1/assets/${id}`, { method: 'DELETE' })
            );
    ...
    const updateMyStatus = async (newStatus) => {
        if (!myTeam) return;
        try {
            await fetch(`${API_URL}/api/v1/teams/${myTeam.id}/deploy`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
