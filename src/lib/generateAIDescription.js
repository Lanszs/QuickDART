// Shared AI description generator used by Dashboard, GuestDashboard, and DroneUpload.
// Converts raw ML analysis output into a meaningful situational report instead of
// a process metadata dump.

export function generateAIDescription(result) {
    if (!result) return "No analysis data available.";

    const type = result.type;
    const damage = result.damage;
    const conf = typeof result.confidence === 'number' ? result.confidence : parseFloat(result.confidence);
    ...
    if (type === 'No Disaster') {
        ...
        parts.push(disasterContext[type] || `A ${type.toLowerCase()} event has been identified in the area.`);
    ...
    if (isVideo && result.type_distribution) {
        ...
        if (activeTypes.length > 1) {
            parts.push(`Video analysis indicates compound disaster indicators: ${activeTypes.map(([n, p]) => `${n} (${p}%)`).join(', ')}. Multiple hazard types may be present simultaneously.`);
        }
    }
    ...
    if (isVideo && result.damage_distribution) {
        ...
        if (activeDamage.length > 1) {
            ...
            if (hasVaried) {
                parts.push(`Damage levels vary across the footage (${activeDamage.map(([n, p]) => `${n}: ${p}%`).join(', ')}), suggesting the affected area has uneven impact zones. Responders should be prepared for mixed conditions.`);
            }
        }
    }
